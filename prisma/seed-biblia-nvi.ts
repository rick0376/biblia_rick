...
const SEED_VERSIONS = (process.env.SEED_VERSIONS ?? "nvi")
  .split(",")
  .map((s) => s.trim().toLowerCase())
  .filter(Boolean) as Version[];

type Version = "nvi" | "ara" | "acf";
...
type Livro = {
  name: string;
  slugApi: string; // abbrev API
  slugDb: string; // slug banco
  testament: Testament;
  order: number;
};
...
/**
 * Verses:
 * - 404 "Chapter not found" => pula (não quebra)
 * - retry em 429/5xx
 */
async function buscarVersiculos(
  ...
  throw new Error(
    `Falha ao buscar versos: ${slugApi} cap ${chapter} (${version})`,
  );
}

/* ===================== DATA ===================== */

const TRANSLATIONS: Record<Version, string> = {
  acf: "Almeida Corrigida Fiel",
  ara: "Almeida Revista e Atualizada",
  nvi: "Nova Versão Internacional",
};

// ✅ 66 livros (slugApi = ABíbliaDigital)
...
async function main(): Promise<void> {
  ...
  console.log(
    `📖 Iniciando seed da Bíblia (${SEED_VERSIONS.join(", ").toUpperCase()})`,
  );

  // cria/upsert das traduções selecionadas
  const translationId = new Map<Version, number>();
  for (const v of SEED_VERSIONS) {
    const tr = await prisma.translation.upsert({
      where: { code: v },
      update: { name: TRANSLATIONS[v] },
      create: { code: v, name: TRANSLATIONS[v] },
    });
    translationId.set(v, tr.id);
  }
  ...
  for (const livro of LIVROS) {
    ...
    for (let cap = 1; cap <= total; cap++) {
      ...
      // traduções: recomendo 1 por vez, mas suporta todas
      for (const v of SEED_VERSIONS) {
        const verses = await buscarVersiculos(v, livro.slugApi, cap);

        if (verses.length > 0) {
          await prisma.verse.createMany({
            data: verses.map((vv) => ({
              chapterId: chapter.id,
              translationId: translationId.get(v)!,
              number: vv.number,
              text: vv.text,
            })),
            skipDuplicates: true,
        ...
        console.log(`   ✔ Capítulo ${cap} (${v})`);
        await sleep(350); // respiro entre versões
      }

      await sleep(900); // respiro entre capítulos
    }
  }
